...
- [ ] main.py 1882 行，需拆分 router
- [ ] RTM 24h 模型回退需修复 (见 docs/rtm-retrain-80features-report.md)
- [ ] 旧 SQLite DB 部分表缺 UNIQUE 约束
- [ ] `prediction/src/data/__init__.py` 的 `__getattr__` 破坏子包导入
- [ ] InfluxDB 仍被 viewer 和 telegram 脚本使用，未完全迁移到 SQLite
- [ ] 废弃的 LaunchAgent plist 文件未清理 (rtm-lmp-scraper, dam-lmp-scraper 等)

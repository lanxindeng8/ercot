#!/usr/bin/env python3
"""DAM v2 model training with CatBoost/LightGBM + Optuna hyperparameter tuning."""
...
def load_data(sp: str):
    """Load train/val/test parquet for a settlement point, drop NaN rows.

    Fuel columns are filled with 0 where missing (fuel data unavailable in
    some periods) rather than dropping those rows entirely.
    """
    sp_dir = DATA_DIR / sp
    dfs = {}
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
    ...
    if mask.sum() > 0:
        ...
        mape = float("nan")
    # Directional accuracy: did we predict the right direction of change vs prior hour?
    if len(y_true) > 1:
...
def naive_baseline_metrics(val_df: pd.DataFrame) -> dict:
    """Naive baseline: yesterday's price (dam_lag_24h) as prediction."""
    mask = val_df["dam_lag_24h"].notna()
    y_true = val_df.loc[mask, TARGET].values
    y_pred = val_df.loc[mask, "dam_lag_24h"].values
...
def _lgbm_objective(trial, X_train, y_train, X_val, y_val):
    ...
    model.fit(
        ...
        callbacks=[_lgbm_early_stopping(100)],
...
def train_settlement_point(sp: str, n_trials: int = 50) -> dict:
    """Train CatBoost + LightGBM for one settlement point, pick the best."""
    log.info(f"=== Training {sp.upper()} ===")
    train_df, val_df, test_df = load_data(sp)
    ...
    # --- Naive baseline ---
    baseline = naive_baseline_metrics(val_df)
    log.info(f"  Naive baseline (val): MAE={baseline['mae']}, MAPE={baseline['mape_pct']}%")

    results = {"settlement_point": sp, "naive_baseline_val": baseline}

    # --- CatBoost with Optuna ---
    log.info(f"  Tuning CatBoost ({n_trials} trials)...")
    t0 = time.time()
    cb_study = optuna.create_study(direction="minimize", study_name=f"{sp}_catboost")
    cb_study.optimize(
    ...
    t0 = time.time()
    lgbm_study = optuna.create_study(direction="minimize", study_name=f"{sp}_lgbm")
    lgbm_study.optimize(
    ...
    results["best_model"] = winner
    log.info(f"  Winner: {winner} (val MAE: CB={cb_val_mae:.4f}, LGBM={lgbm_val_mae:.4f})")

    # --- Save best model ---
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    if winner == "catboost":
        model_path = CHECKPOINT_DIR / f"{sp}_catboost.cbm"
        cb_model.save_model(str(model_path))
        results["model_path"] = str(model_path)
        # Also save test predictions for evaluate.py
        np.save(CHECKPOINT_DIR / f"{sp}_test_preds.npy", cb_test_preds)
    else:
        import joblib
        model_path = CHECKPOINT_DIR / f"{sp}_lightgbm.joblib"
        joblib.dump(lgbm_model, str(model_path))
        results["model_path"] = str(model_path)
        np.save(CHECKPOINT_DIR / f"{sp}_test_preds.npy", lgbm_test_preds)
    ...
    results["top_features"] = [{"feature": f, "importance": round(float(v), 2)} for f, v in fi[:10]]

    log.info(f"  Test metrics ({winner}): {results[winner]['test_metrics']}")
    return results
...
def main():
    ...
    parser.add_argument("--points", nargs="*", default=SETTLEMENT_POINTS,
                        help="Settlement points to train")
    ...
    # Print summary table
    print("\n" + "=" * 80)
    print(f"{'SP':<12} {'Model':<10} {'Val MAE':>10} {'Test MAE':>10} {'Test RMSE':>10} {'MAPE%':>8} {'DirAcc%':>8} {'Baseline':>10}")
    print("-" * 80)
    for sp, r in all_results.items():
        w = r["best_model"]
        tm = r[w]["test_metrics"]
        bl = r["naive_baseline_val"]["mae"]
        print(f"{sp:<12} {w:<10} {r[w]['val_metrics']['mae']:>10.2f} {tm['mae']:>10.2f} {tm['rmse']:>10.2f} {tm['mape_pct']:>8.1f} {tm['directional_accuracy_pct']:>8.1f} {bl:>10.2f}")
    print("=" * 80)

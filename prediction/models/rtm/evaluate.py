#!/usr/bin/env python3
"""RTM multi-horizon model evaluation — per-horizon metrics, peak/off-peak, hourly error."""
...
def _add_rtm_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
...
def load_model(sp: str, target: str):
    """Load the best saved model for a settlement point + horizon."""
    model_tag = f"{sp}_{target}"
    cb_path = CHECKPOINT_DIR / f"{model_tag}_catboost.cbm"
    lgbm_path = CHECKPOINT_DIR / f"{model_tag}_lightgbm.joblib"
    if cb_path.exists():
        ...
        raise FileNotFoundError(f"No model found for {sp}/{target} in {CHECKPOINT_DIR}")
...
def evaluate_horizon(sp: str, target: str, horizon_shift: int) -> dict:
    """Full evaluation for one settlement point and one horizon."""
    log.info(f"--- Evaluating {sp.upper()} / {target} ---")

    model, model_type = load_model(sp, target)

    # Load and prepare test data
    test_df = pd.read_parquet(DATA_DIR / sp / "test.parquet")
    test_df = _create_targets(test_df)
    test_df = _add_rtm_features(test_df)
    ...
    mape = np.mean(np.abs((y_true[mask_nonzero] - y_pred[mask_nonzero]) / y_true[mask_nonzero])) * 100 if mask_nonzero.sum() > 0 else float("nan")

    # Directional accuracy vs naive (lag = horizon)
    lag_col = f"rtm_lag_{horizon_shift}h"
    if lag_col in test_df.columns:
        naive_mask = test_df[lag_col].notna()
        naive_pred = test_df.loc[naive_mask, lag_col].values
        naive_true = test_df.loc[naive_mask, target].values
    ...
    for h in range(24):
        h_mask = hours == h
        if h_mask.sum() > 0:
            hourly_mae[int(h)] = round(float(mean_absolute_error(y_true[h_mask], y_pred[h_mask])), 2)
    ...
    result = {
        ...
        "naive_baseline_mae": round(float(naive_mae), 4) if not np.isnan(naive_mae) else None,
        "improvement_vs_naive_pct": round((1 - mae / naive_mae) * 100, 2) if not np.isnan(naive_mae) and naive_mae > 0 else None,
    ...
    _plot_pred_vs_actual(y_true, y_pred, sp, target)
    _plot_residual_distribution(residuals, sp, target)
    _plot_hourly_mae(hours, y_true, y_pred, sp, target)

    log.info(f"  Overall: MAE={mae:.2f}, RMSE={rmse:.2f}, MAPE={mape:.1f}%, Naive MAE={naive_mae:.2f}")
    return result


def _plot_pred_vs_actual(y_true, y_pred, sp, target):
    ...
    ax.set_xlabel("Actual RTM LMP ($/MWh)")
    ax.set_ylabel("Predicted RTM LMP ($/MWh)")
    ax.set_title(f"{sp.upper()} — {target} — Predicted vs Actual")
    ax.legend()
    ax.set_xlim(lo, hi)
    ax.set_ylim(lo, hi)
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / f"{sp}_{target}_pred_vs_actual.png", dpi=150)
    plt.close(fig)


def _plot_residual_distribution(residuals, sp, target):
    ...
    ax.set_ylabel("Count")
    ax.set_title(f"{sp.upper()} — {target} — Residual Distribution (1st-99th pctl)")
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / f"{sp}_{target}_residual_dist.png", dpi=150)
    plt.close(fig)


def _plot_hourly_mae(hours, y_true, y_pred, sp, target):
    fig, ax = plt.subplots(figsize=(12, 4))
    hourly_mae = []
    for h in range(24):
        mask = hours == h
        if mask.sum() > 0:
            hourly_mae.append(mean_absolute_error(y_true[mask], y_pred[mask]))
        else:
            hourly_mae.append(0)
    ...
    ax.set_title(f"{sp.upper()} — {target} — MAE by Hour of Day")
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / f"{sp}_{target}_hourly_mae.png", dpi=150)
    plt.close(fig)


def evaluate_sp(sp: str) -> dict:
    """Evaluate all horizons for one settlement point."""
    log.info(f"=== Evaluating RTM models for {sp.upper()} ===")
    results = {"settlement_point": sp, "horizons": {}}
    for target, shift in HORIZONS.items():
        try:
            result = evaluate_horizon(sp, target, shift)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate RTM multi-horizon models")
    ...
    # Summary
    print("\n" + "=" * 100)
    print(f"{'SP':<12} {'Horizon':<14} {'MAE':>8} {'RMSE':>8} {'MAPE%':>8} {'Peak MAE':>10} {'OffPk MAE':>10} {'Naive MAE':>10} {'Improv%':>8}")
    print("-" * 100)
    for sp, sp_result in all_results.items():
        for target, r in sp_result["horizons"].items():
            o = r["overall"]
            naive = r.get("naive_baseline_mae", "N/A")
            improv = r.get("improvement_vs_naive_pct", "N/A")
            peak_mae = r["peak"].get("mae", "N/A") if r["peak"] else "N/A"
            offpk_mae = r["offpeak"].get("mae", "N/A") if r["offpeak"] else "N/A"
            naive_str = f"{naive:>10.2f}" if isinstance(naive, float) else f"{naive:>10}"
            improv_str = f"{improv:>8.1f}" if isinstance(improv, float) else f"{improv:>8}"
            peak_str = f"{peak_mae:>10.2f}" if isinstance(peak_mae, float) else f"{peak_mae:>10}"
            offpk_str = f"{offpk_mae:>10.2f}" if isinstance(offpk_mae, float) else f"{offpk_mae:>10}"
            print(f"{sp:<12} {target:<14} {o['mae']:>8.2f} {o['rmse']:>8.2f} {o['mape_pct']:>8.1f} {peak_str} {offpk_str} {naive_str} {improv_str}")
    print("=" * 100)
